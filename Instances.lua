        ...
        Dungeons = {
            ...
            [1841] = { --The Underrot
                ...
                boss1 = {
                    ...
                    achievement = 12498,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = format("Elder Leaxa's abilities apply %s to players whenever they deal damage, which means that all of her abilities will have to be interrupted and evaded in order to prevent this debuff. \n\n To make the achievement easier, you can have your healer go dps so you have an extra interrupt, and also kill her faster.", GetSpellLink(260685)),
                    enabled = true,
                ...
                boss2 = {
                    ...
                    achievement = 12549,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = format("Musashitake is located in the top left corner of the room, and to make him targetable you will have to face the boss cast %s towards his direction. Note: If Musashitake evades attacks then just go the middle of the room, and he will shortly teleport there.", GetSpellLink(272457)),
                    enabled = true,
        ...
        Raids = {
            ...
            [1712] = { --Antorus the Burning Throne
                ...
                boss1 = {
                    ...
                    achievement = 11930,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "For this achievement you will want to give your raid assist so they can place world markers on top of Blightscale Worms. Once someone is targeted by  Decimation, that player will have to move near a worm in order to get it killed.\n\nMake sure you kill the Annihilator in the first intermission, and then slow down the dps on the boss so you don't push him into the second intermission where you'll be forced to kill the Decimator.\n\nNote that you don't have to count how many worms have been killed, just track the achievement and once it turns white, you can kill the boss and get the achievement.",
                    enabled = false,
                    track = nil,
                    partial = false,
                    encounterID = 2076,
                ...
                boss2 = {
                    ...
                    achievement = 12065,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "During the fight several pools of orange and purple color will spawn throughout the room. The tanks will have to bring each felhound to the pool that matches their own color in order to get a stack of  Fel Imbuement. \n\nNote that the  Fel Imbuement stacks get removed if the felhounds come close to each other, so make sure that the tanks are vocal, or have someone calling out where to move the felhounds by using world markers.\n\nOnce both F'harg and Shatug have 5 stacks you can pop  Time Warp, kill the felhounds and you'll get the achievement.",
                    enabled = true,
                ...
                boss3 = {
                    ...
                    achievement = 12129,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "This achievement is pretty straightforward, whoever goes inside the Engineer's Pod must not place more than 3  Felshield Emitters\n\nDon't forget that Admiral Svirax is going to cast  Fusillade 6 times while she is not active so make sure to use raid cooldowns when you run out of  Felshield Emitters.",
                    enabled = true,
                ...
                boss4 = {
                    ...
                    achievement = 11928,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "To open the Chaotic Rift, one player must be affected by all 3 debuffs from each platform at the same time. Bear in mind that this can only be done in Phase 1, which ends when the boss reaches 90%, so make sure not to dps the boss at all.\n\nTo make this achievement easier, you can have one player bring the  Everburning Flames (orange debuff) close to the green platform. After that, another player can pick up the  Cloying Shadows (purple debuff) from the purple platform, and then go to the green platform to pick up the  Everburning Flames (orange debuff) and the  Caustic Slime (green debuff).\n\nCombining these three debuffs will open a chaotic rift which will pull you constantly towards the back of the room.\n\nThe Chaotic Rift assigns random debuffs to players, and in order to prevent this damage from occurring, they will have to go to the platform matching the debuff's color, or else they will explode dealing 5 million damage to themselves:  Binding: Xoroth - Binding: Rancora -  Binding: Nathreza",
                    enabled = true,
                ...
                boss6 = {
                    ...
                    achievement = 11949,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "This achievement is pretty much straightforward, all you have to do is trigger all the traps on the bridge during the intermissions.\n\nIntermission One: On Deadly Ground\nPulse Grenades are the green mines on the ground which deal a burst of nature damage and knock back all nearby players when they are triggered.\nStasis Traps are the green runes on the ground which cause all nearby players to get stunned for 8 seconds when they are triggered.\nPlayers can use defensive cooldowns or immunities to reduce incoming damage, however the stun and knockback effects from either trap will bypass immunities.\nDispels will be necessary and if you have multiple  Priests in your raid group they can rotate  Mass Dispels.\n\nIntermission Two: On Deadlier Ground\nIn addition to the  Pulse Grenades and  Stasis Traps from the previous intermission, you'll also have to trigger  Blastwires and  Shrapnel Blasts:\nBlastwires deal a large amount of damage to the entire raid when they are triggered.\nShrapnel Blasts are the orange mines on the ground which deal damage and apply a dot to anyone nearby when they are triggered.\nDue to the large amount of raid wide damage from triggering Blastwires, it is best to save all significant raid and healing cooldowns for the Second Intermission. Spells such as Tranquillity, Aura Mastery and Revival are great, as you can do a significant amount of healing while on the move.",
                    enabled = false,
                    track = nil,
                    partial = false,
                    encounterID = 2082,
                ...
                boss7 = {
                    ...
                    achievement = 12030,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "For this achievement you will want to have your entire raid group stay in melee range of the boss and only detonate the Diabolic Bombs that spawn in melee.\n\nWhen Phase 2 starts, have your raid use movement increase abilities such as  Stampeding Roar and dodge all bombs on your way to the Garothi constructs. \n\nDiabolic Bombs spawn very frequently throughout the entire fight so you shouldn't worry if you blow up 1 or 2 by mistake. If the boss is at low health and you have 9 active bombs and then the intermission starts, then it's better to just wait for the phase to end and then nuke the boss (use  Time Warp, potions, cooldowns).",
                    enabled = true,
                ...
                boss8 = {
                    ...
                    achievement = 11948,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = " Alone in the Darkness is triggered whenever a player is not within 8 yards of another player. To deal with this mechanic tanks should never move during the entire fight, while melee and ranged stick to their own groups.\n\nVarimathras casts  Marked Prey every 35 to 40 seconds, which will knock back the first target that he gets in contact with. Quite a few classes can deal with this mechanic without triggering  Alone in the Darkness:\nRogue -  Shadowstep\nMonk -  Transcendence -  Transcendence: Transfer\nWarrior - (Spell #3411)\nWarlock -  Demonic Circle",
                    enabled = true,
                ...
                boss9 = {
                    ...
                    achievement = 12046,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "This achievement is pretty straightforward and easy, however how you deal with the adds depends on your RNG:\nIf you get the  Machinations of Aman'Thul as your first, second or third torment then you should kill all of them since your healers might struggle keeping everyone alive, and then wait for another wave to spawn.\nIf you get the  Machinations of Aman'Thul as your fourth torment then this is the ideal scenario as you can use  Time Warp, potions and cooldowns, and then nuke the boss.\nThe  Spectral Army of Norgannon can be crowd controlled, so use abilities such as  Banish,  Freezing Trap or  Entangling Roots\n\nDon't forget to spread out while  Fury of Golganneth is active and move into a safe spot from the  Flames of Khaz'goroth",
                    enabled = false,
                    track = nil,
                    partial = false,
                    encounterID = 2073,
                ...
                boss10 = {
                    ...
                    achievement = 11915,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "For this achievement you will have to transform an Ember of Taeshalach into a Manifestation of Taeshalach. To accomplish that, the Ember has to get hit twice by Aggramar's  Taeshalach Technique combo.\n\nFirst you will want to let the Ember of Taeshalach move away from the edge of the platform, and then have a  Warlock  Banish it. In case you don't have any warlocks, you can use  Freezing Trap,  Entangling Roots or other crowd controls, however make sure that everyone stops dps on the boss to avoid any kind of accidental cleave, which might end up breaking the crowd control.\n\nThe  Taeshalach Technique is  Foe Breaker -  Flame Rend -  Foe Breaker -  Flame Rend -  Searing Tempest\nYou can set focus on the Ember of Taeshalach and check its debuffs to see if any ability has missed. \n\nNote that you have to transform the Ember of Taeshalach within a single phase or else it will detonate, and because of this you should do it in Stage 2: Champion of Sargeras (80%-40%).",
                    enabled = true,
                ...
                boss11 = {
                    ...
                    achievement = 12257,
                    players = {L["(Enter instance to start scanning)"]},
                    tactics = "For this achievement you will first have to get to Phase 4 and then bring down Argus to about 5% health.\n\nNow whenever Argus spawns of 5 or 6 Reorigination Modules (up to your preference), you will have to kill them and then have several dps die from the  Reorigination Pulse or  Deadly Scythe.\n\nOn the next wave of Reorigination Modules, use  Time Warp, potions, cooldowns and nuke all of them.\n\nAssign one person inside the spirit world to make a short countdown so that everyone starts picking up motes at the exact same time. Use the Weak Aura below to track Khaz'goroth's energy, and if your raid group picked up enough motes within 5 seconds, it will inform you about that in raid chat. Once the requirement is met, you can kill the boss and get the achievement.",
                    enabled = true,
                    track = function() end,
                    partial = false,
                    encounterID = 2092,
