        ...
        if core.destName ~= nil then
            ...
            if UnitIsPlayer(name) then
                ...
                if core.spellId == 260279 then
                    reason = format(L["Shared_DamageFromAbility"], C_Spell.GetSpellLink(260279))
                elseif core.spellId == 276234 then
                    reason = format(L["Shared_DamageFromAbility"], C_Spell.GetSpellLink(276234))
                elseif core.spellId == 270926 then
                    reason = format(L["Shared_DirectHitFromAbility"], C_Spell.GetSpellLink(270926))
                elseif core.spellId == 270277 then
                    reason = format(L["Shared_DirectHitFromAbility"], C_Spell.GetSpellLink(270277))
                end
