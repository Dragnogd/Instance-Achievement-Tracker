--[[
type:
    all - use sparingly because achievements look havy
...
lib.events = {
    ...
    achievements = "INSPECT_ACHIEVEMENT_READY",
}
    ...
    if addon and what and callback then
        if type(what) == 'string' then
            if what == 'all' then
                ...
                local a = self:SecureAddHook(addon, 'achievements', callback);
            ...
            elseif what == 'achievements' then
                return self:SecureAddHook(addon, 'achievements', callback);
            else
...
function lib:RemoveHook(addon, what)
    if addon then
        ...
        if what == 'all' then
            ...
            self:RemoveHook(addon, 'achievements');
        elseif what == 'items' or what == 'honor' or what == 'talents' or what == 'achievements' then
    ...
    if guid then
        ...
        if self.cache[guid].data == false or self.cache[guid].time == 0 or (time() - self.cache[guid].time) > self.maxAge or force then
            ...
            if what == 'all' then
                ...
                self:SafeRequestachievements(target, guid, skip);
            elseif what == 'items' then
                self:SafeRequestItems(target, guid, skip);
            elseif what == 'honor' then
                self:SafeRequestHonor(target, guid, skip);
            ...
            if what == 'all' then
                ...
                self:RunHooks('achievements', guid);
            elseif what == 'items' then
                self:RunHooks('items', guid);
            elseif what == 'honor' then
                self:RunHooks('honor', guid);
...
function lib:Requestachievements(target, force) return self:RequestData('achievements', target, force); end
...
function lib:SafeRequestachievements(target, guid, skip)
    print("Requesting achievements")
end
